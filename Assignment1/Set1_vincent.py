...
class string_leap:
    ...
    def run_regular(self, t_max, start_con):
        ...
        while self.t < t_max:
            self.string_regular_step()
            self.wave_history.append(self.y_pos.copy())
            self.time_history.append(self.t)

    def run_leap(self, t_max, start_con):
          # Store all time steps
        ...
        if start_con == 3:
            for i in range(len(self.x_points)):
                if self.x_points[i] > 0.2 and self.x_points[i] < 0.4:
                    self.y_pos[i] = np.sin(5 * np.pi * (self.x_points[i]))
                else:
                    self.y_pos[i] = 0
    ...
    def data_plotter(self, time_list, num_plots=4):
        plt.figure(figsize=(10, 6))
        for i in time_list: #range(0, len(self.wave_history), max(1, len(self.wave_history) // num_plots)):
...
string2.run_leap(2, 3)
#string2.animate_wave() #uncomment for the gif
string2.data_plotter([0, 1200, 2000, 3500])
